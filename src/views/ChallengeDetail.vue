...
const editChallenge = async (e) => {
    e.preventDefault()
    await axios.post(`/api/challenge/update/${route.params.id}`,
    {
      nama_tantangan: nama_tantangan.value,
  ...
  Swal.fire("Successfully edited Challenge");
  isDisabled.value = !isDisabled.value;
  isEditing.value = !isEditing.value;
  challengeDetail();
};

const challengeDetail = async () => {
    const response = await axios.get(`/api/challenge/${route.params.id}`);
    nama_tantangan.value = response.data.data.nama_tantangan;
    deskripsi.value = response.data.data.deskripsi;
    tanggal_mulai.value = response.data.data.tanggal_mulai;
    tanggal_berakhir.value = response.data.data.tanggal_berakhir;
    challenges.value = response.data.data;
}

onMounted(challengeDetail)

const deleteChallenge = async (e) => {
  e.preventDefault()
  if(confirm("Are you sure you want to delete this Challenge?")) {
    await axios.delete(`/api/challenge/delete/${route.params.id}`);
    Swal.fire("Successfully deleted Challenge");
      ...
      <div class="body">
        <div class="max-w-8xl mx-auto px-4 mt-6">
          <div class="items-center">
            <div v-if="!isEditing">
                <h1 class="inline-block text-4xl font-semibold text-emerald-700 tracking-tight">{{ nama_tantangan }}</h1>
            </div>
            
            <RouterLink to="/challenge">
              <Button class="bg-emerald-700 px-3 py-2 mt-5 rounded-md hover:-translate-y-1 duration-200">
                <p class="text-white font-semibold" >Back to Challenge</p>
              </Button>
            </RouterLink>
            ...
            <form class="w-full max-w-lg mt-10">
                <div class="flex flex-wrap -mx-3 mb-6">
                    <div class="w-full px-3">
                        ...
                        <input :disabled="isDisabled" v-model="nama_tantangan" class="appearance-none block w-full bg-gray-200-50  text-gray-700 border border-emerald-700 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"id="grid-first-name" type="text" placeholder="Learn Python">
                    </div>
                </div>
                <div class="flex flex-wrap -mx-3 mb-6">
                    <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
                        ...
                        <input :disabled="isDisabled" v-model="tanggal_mulai" class="appearance-none block w-full bg-gray-200-50  text-gray-700 border border-emerald-700 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"id="grid-first-name" type="date" placeholder="Sad">
                    </div>
                    <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
                        <label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" for="grid-first-name">
                            Finish Date
                        </label>
                        <input :disabled="isDisabled" v-model="tanggal_berakhir" class="appearance-none block w-full bg-gray-200-50  text-gray-700 border border-emerald-700 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"id="grid-first-name" type="date" placeholder="#ffffff">
                        <p class="text-xs text-gray-200-50">Pick the date later when you have finished your task.</p>
                    </div>
                </div>
                <div class="flex flex-wrap -mx-3 mb-6">
                    <div class="w-full px-3">
                ...
                <div class="md:w-2/3">
                    ...
                    <button v-else @click="editChallenge" class="shadow bg-emerald-700 hover:bg-emerald-600 focus:shadow-outline focus:outline-none text-white font-bold py-2 px-4 rounded" type="submit">
                        Save
                    </button>
                    <button @click="deleteChallenge" class="shadow bg-white border border-emerald-700 hover:bg-slate-50; focus:shadow-outline focus:outline-none text-amber-500 font-bold py-2 px-4 ml-3 rounded">
                      Delete
                    </button>
                </div>
            </form>
