...
const editJournal = async (e) => {
    e.preventDefault()
    await axios.post(`/api/journal/update/${route.params.id}`,
    {
      title: title.value,
      content: content.value,
    });
 
  Swal.fire("Successfully edited Journal");
  isDisabled.value = !isDisabled.value;
  isEditing.value = !isEditing.value;
  journalDetail();
};
...
onMounted(journalDetail)

const deleteJournal = async (e) => {
  e.preventDefault()
  if(confirm("Are you sure you want to delete this journal?")) {
    await axios.delete(`/api/journal/delete/${route.params.id}`);
    Swal.fire("Successfully deleted Journal");
    router.push("/journal");
  }
}
    ...
    <div class="home">
      ...
      <div class="body">
        <div class="max-w-8xl mx-auto px-4 mt-6">
          <div class="items-center page-custom">
            <div v-if="!isEditing">
                <h1 class="inline-block text-4xl font-semibold text-emerald-700 tracking-tight">{{ title }}</h1>
            </div>

            <div>
              <RouterLink to="/journal">
                <Button class="bg-emerald-700 px-3 py-2 mt-5 rounded-md hover:-translate-y-1 duration-200">
                    <p class="text-white font-semibold" >Back to Journal</p>
                </Button>
              </RouterLink>
            </div>
            ...
            <form class="w-full max-w-lg mt-10">
                <div class="flex flex-wrap -mx-3 mb-6">
                    <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
                        ...
                        <input :disabled="isDisabled" v-model="title" class="appearance-none block w-full bg-gray-200-50  text-gray-700 border border-emerald-700 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"id="grid-first-name" type="text" placeholder="Jane">
                    </div>
                </div>
                <div class="flex flex-wrap -mx-3 mb-6">
                    <div class="w-full px-3">
                        ...
                        <textarea :disabled="isDisabled" v-model="content" class="resize rounded-sm appearance-none block w-full bg-gray-200-50 text-gray-700 border border-emerald-700 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" ></textarea>
                ...
                <div class="md:w-2/3">
                    ...
                    <button v-else @click="editJournal" class="shadow bg-emerald-700 hover:bg-emerald-600 focus:shadow-outline focus:outline-none text-white font-bold py-2 px-4 rounded">
                        Save
                    </button>
                    <button @click="deleteJournal" class="shadow bg-white border border-emerald-700 hover:bg-slate-50; focus:shadow-outline focus:outline-none text-amber-500 font-bold py-2 px-4 ml-3 rounded">
                      Delete
                    </button>
                </div>
            </form>         
