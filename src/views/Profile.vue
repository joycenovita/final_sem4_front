...
const editProfile = async () => {
  ...
  Swal.fire("Successfully edited Profile");
  isDisabled.value = !isDisabled.value;
  isEditing.value = !isEditing.value;
  profile();
};
...
const deleteProfile = async (e) => {
  ...
  if(confirm("Are you sure you want to delete this user?")) {
    ...
    Swal.fire("We are sorry to here you go..")
    router.push("/sign-in");
  }
}
      ...
      <div class="body">
        <div class="max-w-8xl mx-auto px-4 mt-6">
          <div class="items-center">
            <h1 class="inline-block text-4xl  font-semibold text-emerald-700 tracking-tight">PROFiLE</h1>

            <p class="mt-2">
              Hi there! We're glad that you came here to express yourself more. Please fill out these forms, we want to get to know more about you!
            </p>
            ...
            <form class="w-full max-w-lg mt-10">
                <div class="flex flex-wrap -mx-3 mb-6">
                    <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
                        ...
                        <input :disabled="isDisabled" v-model="first_name" class="appearance-none block w-full bg-gray-200-50  text-gray-700 border border-emerald-700 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"id="grid-first-name" type="text" placeholder="Jane">
                    </div>
                    <div class="w-full md:w-1/2 px-3">
                        <label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" for="grid-last-name">
                            Last Name
                        </label>
                        <input :disabled="isDisabled" v-model="last_name" class="appearance-none block w-full bg-gray-200-50 text-gray-700 border border-emerald-700 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="grid-last-name" type="text" placeholder="Doe">
                    </div>
                </div>
                <div class="flex flex-wrap -mx-3 mb-6">
                    <div class="w-full px-3">
                        ...
                        <input :disabled="isDisabled" v-model="email" class="appearance-none block w-full bg-gray-200-50  text-gray-700 border border-emerald-700 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="grid-password" type="email">
                    </div>
                </div>
            </form>
            <div class="md:w-2/3">
              ...
              <button v-else @click="editProfile" class="shadow bg-emerald-700 hover:bg-emerald-600 focus:shadow-outline focus:outline-none text-white font-bold py-2 px-4 rounded">
                Save
              </button>
              <button @click="deleteProfile" class="shadow bg-amber-500 hover:bg-amber-600 focus:shadow-outline focus:outline-none text-white font-bold py-2 px-4 ml-3 rounded">
                Delete
              </button>
             </div>
          </div>
